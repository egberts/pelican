...
class TestSettingsLoadSourcePath:
    ...
    def test_load_source_path_str_name_blank_fail(self):
        """one blank string argument; failing mode"""
        with pytest.raises(TypeError) as sample:
            load_source(name="")  # NOQA: RUF100
        assert sample.type == TypeError

    def test_load_source_path_str_both_blank_fail(self):
        """one blank string argument; failing mode"""
        with pytest.raises(SyntaxError) as sample:
            load_source(None, path=None)  # NOQA: RUF100
        assert sample.type == SyntaxError

    def test_load_source_path_str_path_blank_fail(self):
        """one blank string argument; failing mode"""
        with pytest.raises(SyntaxError) as sample:
            load_source(None, path="")  # NOQA: RUF100
        assert sample.type == SyntaxError
    ...
    def test_load_source_path_str_path_none_fail(self):
    ...
    def test_load_source_path_arg_list_fail(self):
        """argument name with blank str; failing mode"""
        with pytest.raises(TypeError) as sample:
            load_source(name="", path=[])
        assert sample.type == TypeError

    def test_load_source_path_arg_dict_fail(self):
        """argument name with blank str; failing mode"""
        with pytest.raises(TypeError) as sample:
            load_source(name="", path={})
        assert sample.type == TypeError
    ...
    def test_load_source_path_name_none_unexpected_type_list_fail(self):
        """invalid dict argument type with argument name; failing mode"""
        path_list = {}
        with pytest.raises(TypeError) as sample:
            load_source(name=None, path=path_list)  # NOQA: RUF100
        assert sample.type == TypeError
