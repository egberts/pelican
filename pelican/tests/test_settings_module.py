...
class TestSettingsModuleName:
    ...
    def test_load_source_name_dict_fail(self, fixture_func_ut_wrap):
        """arguments all None, failing mode"""
        # Let the load_source() determines the module name from its path
        dict_module = {}
        valid_path = PC_FILENAME_VALID

        with pytest.raises(TypeError) as sample:
            load_source(name=dict_module, path=valid_path)

        assert sample.type == TypeError

    def test_load_source_all_none_fail(self, fixture_func_ut_wrap):
        """arguments all None, failing mode"""
        # Let the load_source() determines the module name from its path
        empty_module = None
        empty_path = None
    ...
    def test_load_source_str_rel_no_access_fail(
        ...
        # Let the load_source() determines the module name from its path
        valid_module = PC_MODNAME_VALID
        module_not_expected_in_sys_modules(valid_module)
        ...
        with self._caplog.at_level(logging.DEBUG):
            ...
            with pytest.raises(PermissionError) as sample:
                load_source(valid_module, noaccess_rel_filespec_str)

            assert sample.type == PermissionError
            assert "is not readable" in str(sample.value)
    ...
    def test_load_source_str_abs_no_access_fail(
        ...
        with self._caplog.at_level(logging.DEBUG):
            self._caplog.clear()
            with pytest.raises(PermissionError) as sample:
                load_source(valid_module, str(noaccess_abs_filespec_path))
